...
async function getTeamsList() {
    ...
    _get(`/api/v1/data/detail/${idNumber}`).then((resJson: { "FullMain": { "id": number, "event": string, "season": number, "team": number, "match_num": number, "level": string, "game": string, "defend": string, "driving": string, "overall": String, "user_id": number, "name": string, "from_team": number, "weight": string, "analysis": string }}[]) => {
        const gameData: { "intake": number, "travel": number, "outtake": number, "score_type": number }[] = JSON.parse(resJson[0].FullMain.game);
        ...
        htmlContent += `</div>`;
        (document.getElementById("insertHere") as HTMLTableRowElement).insertAdjacentHTML("afterbegin", htmlContent);
        (document.getElementById("eventCodeDisplay") as HTMLHeadingElement).innerText = `Team ${resJson[0].FullMain.team} | Match ${resJson[0].FullMain.match_num} @ ${resJson[0].FullMain.event} 2025`;
