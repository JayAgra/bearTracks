...
let buttons = Array.from(document.getElementsByClassName("actionButton")) as Array<HTMLButtonElement>;

async function getTeamsList() {
    disableButton(0)
    showPage(0)
    document.getElementsByTagName("tbody")[0].replaceChildren(document.getElementsByTagName("tbody")[0].firstElementChild)
    _get(`/api/v1/data/teams/2025/${getEventCookie()}`).then((resJson: { "Team": { "id": Number, "team": Number, "weight": String }}[]) => {
        resJson.sort((a, b) => {
...
async function getMatchesList() {
    disableButton(1);
    showPage(0);
    (document.getElementById("preInsertMatch") as HTMLTableRowElement).innerHTML = ""
    _get(`/api/v1/events/matches/2025/${getEventCookie()}/qualification/all`).then((resJson: { Schedule: { description: String, startTime: String, matchNumber: String, field: String, tournamentLevel: String, teams: { teamNumber: number, station: String, surrogate: boolean }[]; }[]; }) => {
        var htmltable = "";
        ...
        (document.getElementById("preInsertMatch") as HTMLTableRowElement).insertAdjacentHTML("afterbegin", htmltable + "<hr>");
        showPage(2);
    }).catch((error: any) => console.log(error));
}

function showPage(page: number) {
    pages[0].style.display = "none"; pages[1].style.display = "none"; pages[2].style.display = "none";
    pages[page].style.display = "flex";
}

function disableButton(button: number) {
    buttons[0].removeAttribute("disabled"); buttons[1].removeAttribute("disabled");
