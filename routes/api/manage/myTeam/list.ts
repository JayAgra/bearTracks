...
export async function teamScouts(req: express.Request, res: express.Response, authDb: sqlite3.Database) {
    if (req.user.teamAdmin === 0) return res.status(403).send("" + 0x1931);
    const stmt: string = `SELECT id, nickName, score, accessOk FROM users WHERE team=? ORDER BY score DESC`;
