        ...
        if (xhr.readyState === XMLHttpRequest.DONE && xhr.status === 200) {
            ...
            for (var i = 0; i < resJson.length; i++) {
                if (resJson[i].accessOk == "true") {
                    htmltable += `<tr><td><a href="/browse?userId=${resJson[i].id}" style="all: unset; color: #2997FF; text-decoration: none;">${resJson[i].nickName} (${resJson[i].team})</a></td><td>${Math.round(resJson[i].score)}</td></tr>`;
                }
            }
