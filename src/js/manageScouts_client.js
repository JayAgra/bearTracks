...
async function getData() {
    ...
    try {
        ...
        for (var i = 0; i < listRes.length; i++) {
            listHTML += `<tr class="padded"><td>${listRes[i].username}</td><td>${listRes[i].score}</td><td><div class="inlineInput"><input type="tel" id="${listRes[i].discordID}_input" value="${listRes[i].score}"><button class="uiButton actionButton" onclick="updateUser('${listRes[i].discordID}', ${listRes[i].score}, this)">save</button></div></td></tr>`;
        }
