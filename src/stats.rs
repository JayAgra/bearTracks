pub fn quartiles_i64(data: &Vec<i64>) -> Vec<i64> {
    if data.len() > 0 {
        ...
        if (data_length * 0.25) % 1.0 == 0.0 {
            quartiles.push(
                (sorted_data[((data_length * 0.25) - 1.0) as usize]
                    + sorted_data[(data_length * 0.25) as usize])
                    / 2 as i64,
            );
        } else {
        ...
        if (data_length * 0.5) % 1.0 == 0.0 {
            quartiles.push(
                (sorted_data[((data_length * 0.5) - 1.0) as usize]
                    + sorted_data[(data_length * 0.5) as usize])
                    / 2 as i64,
            );
        } else {
        ...
        if (data_length * 0.75) % 1.0 == 0.0 {
            quartiles.push(
                (sorted_data[((data_length * 0.75) - 1.0) as usize]
                    + sorted_data[(data_length * 0.75) as usize])
                    / 2 as i64,
            );
        } else {
...
pub fn means_i64(data: &Vec<i64>, first_wt: f64) -> Vec<i64> {
    if data.len() > 0 {
        ...
        means.push(
            ((data[0] as f64 * first_wt) + (data.iter().sum::<i64>() as f64 * (1.0 - first_wt)))
                as i64,
        );
        return means;
    }
