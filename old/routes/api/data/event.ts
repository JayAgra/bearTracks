...
export async function getAllEventData(req: express.Request, res: express.Response, db: sqlite3.Database, season: number) {
    const stmt: string = "SELECT id, team, match, level, game2, game3, game4, game5, game6, game7, game8, game10, game25, game11, weight, userId, name, fromTeam FROM main WHERE season=? AND event=? ORDER BY id DESC";
